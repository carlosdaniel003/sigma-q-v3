...
export default function DiagnosticoAvancado({ stats }: { stats: any }) {
  ...
  return (
    <section className="diag-adv-card fade-in">
      ...
      {/* TÍTULO */}
      <h4 className="diag-adv-title">🩺 Diagnóstico Avançado do Sistema</h4>
        ...
        <table className="diag-adv-table">
          ...
          <tbody>
            {["af", "lcm", "produto acabado", "pth"].map((k) => {
              ...
              return (
                <tr key={k}>
                  <td className="bold">{k.toUpperCase()}</td>
                  <td>{b.total ?? 0}</td>
                  <td className="green">{b.identified ?? 0}</td>
                  <td className={b.notIdentified > 0 ? "red" : "muted"}>
                    {b.notIdentified ?? 0}
                  </td>
                  <td>{pct.toFixed(2)}%</td>
                </tr>
      ...
      <div className="diag-adv-insight">
        ...
        <div className="diag-adv-insight-list">
          {(() => {
            ...
            /* =====================================================
               2) PARETO — MAIOR RESPONSÁVEL PELOS ERROS
            ====================================================== */
